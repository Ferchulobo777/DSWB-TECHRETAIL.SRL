...
exports.crearPedido = async (req, res, next) => {
  try {
    const { usuarioId, tiendaId, fecha, productos } = req.body;
    ...
    const numeroOrden = (ultimoPedido && ultimoPedido.numeroOrden) ? ultimoPedido.numeroOrden + 1 : 1;
...
exports.actualizarPedido = async (req, res, next) => {
  try {
    ...
    const pedidoActualizado = await Pedido.findByIdAndUpdate(
      req.params.id,
      { usuarioId, tiendaId: tiendaId || undefined, productos, fecha: fecha || Date.now(), total },
      { new: true }
    );
